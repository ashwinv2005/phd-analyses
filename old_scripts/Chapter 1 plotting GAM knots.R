...
ggp = ggplot(temp1, aes(x = status1, y = predm, col = as.factor(plotx)))  +
  ...
  geom_smooth(method = "gam",
              formula = y ~ s(x, bs = "cs", k = 4), se = F) +
  geom_smooth(aes(x = status1, y = sel, col = as.factor(plotx)), method = "gam",
  formula = y ~ s(x, bs = "cs", k = 4), se = F) +
  
  geom_smooth(aes(x = status1, y = ser, col = as.factor(plotx)), method = "gam",
  formula = y ~ s(x, bs = "cs", k = 4), se = F)
...
ggp + 
  ...
  #scale_x_continuous(breaks = c(0,25,50,75,100,125,150,175,200,225,250,275,300), limits = c(0,300)) +
