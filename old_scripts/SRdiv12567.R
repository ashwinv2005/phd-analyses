...
sl1 = sggro1m[,-c(2,6,12,28)]
sl2 = sggro2m[,-c(2,6,12,28)]

sl1 = sggro1m[,-c(2,6,28)]
sl2 = sggro2m[,-c(2,6,28)]

y = table(sggroup[sggroup$status1 != 0,]$species)
y = y[y>=5]

temp = names(sggro1m)
x = temp %in% names(y) & temp != "SR" & temp != "Symp" & temp != "Climber1" & temp != "SC"
