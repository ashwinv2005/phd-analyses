...
p = grid_arrange_shared_legend(ggp1, ggp2, ggp3, ggp4)
...
g = gridExtra::arrangeGrob(p,ncol=1, 
                           left=grid::textGrob("change in dissimilarity - germination to establishment", rot=90, gp = gpar(fontfamily = "serif", fontsize = 8, col = 'black'))) ; 
#grid::grid.newpage() ; 
...
#data = temp4[temp4$newbrayw > -0.1 & temp4$newbrayw < 0.3,],

fit9d = lmer (brayw ~ bray1w + (plot57 + plot67)*distm + (plot57 + plot67)*msizem + (plot57 + plot67)*(msizem:sizem) + 
                (1|com1/plot1) + (1|com2/plot2), 
              data = temp, contrasts = list(plot57 = cMat, plot67 = cMat))

anova(fit9c,fit9d)

fit9c = lmer (log(-diff) ~ log(dis1) + (plot57 + plot67)*log(distm) + (plot57 + plot67)*smsizem + 
                (plot57 + plot67)*(smsizem:sizem) + (1|com1/plot1) + (1|com2/plot2),
              data = temp[temp$diff < 0,], contrasts = list(plot57 = cMat, plot67 = cMat))

fit9c = lmer (log(-diffw) ~ log(dis1w) + (plot57 + plot67)*log(distm) + (plot57 + plot67)*smsizem + 
                (plot57 + plot67)*(smsizem:sizem) + (1|com1/plot1) + (1|com2/plot2),
              data = temp[temp$diffw < 0,], contrasts = list(plot57 = cMat, plot67 = cMat))
...
plot(temp[!is.na(temp$brayw),]$distm, resid(fit8)) # residuals vs. predictor 1
plot(temp[!is.na(temp$brayw),]$sizem, resid(fit8)) # residuals vs. predictor 2
