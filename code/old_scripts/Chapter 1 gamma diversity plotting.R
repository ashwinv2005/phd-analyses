...
ggp3 = ggp + 
  ggtitle("all species") +
  theme(plot.title = element_text(hjust = 0.5, size = 9)) +
  theme(axis.title.x = element_text(vjust = 0.5, size = 8), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  theme(legend.title = element_text(size = 0, face = "bold"), legend.text = element_text(size = 8))+
  #theme(strip.background = element_rect(colour=NULL, fill=NA)) +
  theme(strip.text.x = element_text(size = 10, face = "bold")) +
...
p = grid_arrange_shared_legend(ggp1, ggp2, ggp3, ggp4, ggp5, ggp6)
...
g = gridExtra::arrangeGrob(p,ncol=1, 
                           left=grid::textGrob("change in gamma diversity - germination to establishment", rot=90, gp = gpar(fontfamily = "serif", fontsize = 8, col = 'black'))) ; 
#grid::grid.newpage() ; 
