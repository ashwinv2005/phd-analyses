...
p1 = grid_arrange_shared_legend1(ggp1)
p2 = grid_arrange_shared_legend2(ggp2)
...
g = gridExtra::arrangeGrob(p1,p2,nrow=2,ncol=1, 
                           left=grid::textGrob("change in alpha diversity - germination to establishment", rot=90, gp = gpar(fontfamily = "serif", fontsize = 12, col = 'black'))) ; 
#grid::grid.newpage() ; 
...
ggp = ggplot(temp1, aes(x = size, y = predm, col = as.factor(plotx)))  +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = F) +
  geom_smooth(aes(x = size, y = sel, col = as.factor(plotx)), method = "lm", se = F) +
  
  geom_smooth(aes(x = size, y = ser, col = as.factor(plotx)), method = "lm", se = F)
...
ggp = ggplot(temp2, aes(x = size, y = predm, col = as.factor(plotx)))  +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = F) +
  geom_smooth(aes(x = size, y = sel, col = as.factor(plotx)), method = "lm", se = F) +
...
library(grid)
pd = position_dodge(1)
t2 = summarySE(temp2, groupvars = c("plotx","size"), measurevar = "newtransI")


ggpw = ggplot(temp2[temp2$plotx == "1",], aes(x = size,  col = as.factor(plotx), 
                                              fill = as.factor(plotx)))  +
  geom_point(data = t2[t2$plotx == "1",], aes(x = size, y = (newtransI)), 
             size = 1, position = pd) +
  geom_errorbar(data = t2[t2$plotx == "1",], aes(x = size, ymin = newtransI - se,ymax = newtransI + se), size = 0.2, 
                width = 1, position = pd) +
  geom_smooth(aes(x = size, y = predm), method = "lm", se = F, size = 0.5) +
  geom_ribbon(aes(x = selribx, ymin = selriby,ymax = serriby, linetype = NA), alpha=0.1) +
...
ggp = ggplot(temp2, aes(x = size,  col = as.factor(plotx), 
                                             fill = as.factor(plotx)))  +
  geom_point(data = t2, aes(x = size, y = (newtransI)), 
             size = 1, position = pd) +
  geom_errorbar(data = t2, aes(x = size, ymin = newtransI - se,ymax = newtransI + se), size = 0.2, 
                width = 1, position = pd) +
  geom_smooth(aes(x = size, y = predm), method = "lm", se = F, size = 0.5) +
  geom_ribbon(aes(x = selribx, ymin = selriby,ymax = serriby, linetype = NA), alpha=0.1) +
  xlab("fragment size (ha)") +
...
############## presentation control fungicide ############## repeat with insecticide

library(gridExtra)
library(grid)

ggpw = ggplot(temp2[temp2$plotx == "1" | temp2$plotx == "6",], aes(x = size,  col = as.factor(plotx), 
                                              fill = as.factor(plotx)))  +
  #geom_point(data = t2[t2$plotx == "1" | t2$plotx == "6",], aes(x = size, y = (newtransI)), 
  #           size = 1, position = pd) +
  #geom_errorbar(data = t2[t2$plotx == "1" | t2$plotx == "6",], aes(x = size, ymin = newtransI - se,ymax = newtransI + se), size = 0.2, 
  #              width = 1, position = pd) +
  geom_smooth(aes(x = size, y = predm), method = "lm", se = F, size = 0.5) +
  geom_ribbon(aes(x = selribx, ymin = selriby,ymax = serriby, linetype = NA), alpha=0.1) +
  xlab("fragment size (ha)") +
...
############## presentation all ############## repeat without points

library(gridExtra)
library(grid)

ggpw = ggplot(temp2, aes(x = size,  col = as.factor(plotx), 
                                                                   fill = as.factor(plotx)))  +
  #geom_point(data = t2, aes(x = size, y = (newtransI)), 
  #           size = 1, position = pd) +
  #geom_errorbar(data = t2, aes(x = size, ymin = newtransI - se,ymax = newtransI + se), size = 0.2, 
...
library(gridExtra)
library(grid)
t1 = summarySE(temp1, groupvars = c("plotx","size"), measurevar = "newtrans")

ggpw = ggplot(temp1, aes(x = size,  col = as.factor(plotx), 
                         fill = as.factor(plotx)))  +
  #geom_point(data = t1, aes(x = size, y = (newtrans)), 
  #           size = 1, position = pd) +
  #geom_errorbar(data = t1, aes(x = size, ymin = newtrans - se,ymax = newtrans + se), size = 0.2, 
  #              width = 1, position = pd) +
  geom_smooth(aes(x = size, y = predm), method = "lm", se = F, size = 0.5) +
  geom_ribbon(aes(x = selribx, ymin = selriby,ymax = serriby, linetype = NA), alpha=0.1) +
  xlab("fragment size (ha)") +
...
ggpw1 = ggpw + 
  ...
  scale_size(guide = 'none') +
  scale_x_continuous(limits = c(1,150)) +
  scale_y_continuous(limits = c(-1.2,0.35)) +
  scale_color_manual(values = c("#56B4E9","#D55E00","#009E73","#CC79A7"),
                     name="",
                     breaks=c("1","6","5","7"),
                     labels=c("control","insecticide","fungicide","combined")) +
...
ggp = ggplot(temp1, aes(x = size,  col = as.factor(plotx), 
                        fill = as.factor(plotx)))  +
  geom_point(data = t1, aes(x = size, y = (newtrans)), 
             size = 1, position = pd) +
  geom_errorbar(data = t1, aes(x = size, ymin = newtrans - se,ymax = newtrans + se), size = 0.2, 
