...
temp$stage = c(rep("adults",8),rep("seeds",8),rep("young saplings",8),rep("old saplings (> 1cm girth)",8))
temp = temp[,-1]
temp$div = NA
...
temp$stage = factor(temp$stage, levels = c("adults","seeds","young saplings","old saplings (> 1cm girth)"))
...
ggp1 = ggp +
  theme(axis.title.x = element_text(vjust = 0.3, size = 8), axis.text.x = element_text(size = 6),
        axis.title.y = element_text(vjust = 0.3, angle = 90, size = 8), axis.text.y = element_text(size = 6)) +
...
ggp = ggplot(can, aes(x = size, y = canopy))  +
  geom_point(size = 1) +
  geom_errorbar(aes(ymin = canopy-ci, ymax = canopy+ci), width = 0.1, size = 0.1) +
  geom_smooth(data = canopy, aes(x = size, y = canopy), method = "lm", alpha = 0.1, size = 0.4) +
  xlab("fragment size (ha)") +
  ylab("proportion canopy cover") 
ggp3 = ggp +
  theme(axis.title.x = element_text(vjust = 0.3, size = 10), axis.text.x = element_text(size = 8),
        axis.title.y = element_text(vjust = 0.3, angle = 90, size = 10), axis.text.y = element_text(size = 8)) +
  theme(legend.title=element_blank(), legend.text = element_text(size = 8))+
  #theme(strip.background = element_rect(colour=NULL, fill=NA)) +
  scale_size(guide = 'none') +
